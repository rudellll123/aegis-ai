...
<p align="center">
  ...
  <a href="#️-development-phases--all-12-shipped">🗺️ Phases</a> ·
...
<p align="center">
  <img src="assets/demo.gif" alt="AegisAI Command Center Dashboard demo — live incident feed, video and audio evidence upload, real-time engine status" width="850">
</p>

<p align="center"><sub>🟢 Render API · 🟢 Vision Engine · 🟢 RAG Engine · 🟢 Audio Engine — all pulled live from the deployed backend, not mocked.</sub></p>

This is the **Command Center** running end to end: incident feed streamed from PostgreSQL, a video clip run through the real YOLOv8 + ByteTrack pipeline, and an audio file transcribed through Whisper — all triggered from the browser, all hitting the live agent stack.

🔗 **Try it yourself:** `https://YOUR-RENDER-SERVICE-URL.onrender.com` · 🖥️ **Dashboard repo path:** `frontend/` (see [Setup & Run](#-setup--run))
...
⚠️ **Positioning matters:** AegisAI is AI-*assisted*, not autonomous. Human approval stays in the loop at every single stage — see the "Human Review Enabled" badge live on the dashboard above.
...
Every request flows straight down this stack, one piece of evidence at a time — from the user, through the gateway and supervisor agent, out to whichever specialist tools the agent picks, and back up through synthesis, observability, and human sign-off:

<p align="center">
  <img src="assets/architecture.png" alt="AegisAI system architecture — User to Command Center Dashboard to FastAPI Gateway to LangGraph Supervisor Agent to MCP Client to MCP Tool Server, fanning out to RAG, Vision, Data, Knowledge, and Audio agents, into Report Synthesis, Observability, and Human Review" width="700">
</p>

| Layer | Component | Role |
|---|---|---|
| 👤 | **User** | Initiates a request from the dashboard |
| 🖥️ | **Command Center Dashboard** — Next.js · React · TypeScript · Tailwind | Live UI: incidents, uploads, pipeline status |
| 🌐 | **FastAPI Gateway** — deployed live on Render | Entry point for all API traffic |
| 🧠 | **LangGraph Supervisor Agent** | Routes each request to the right tool |
| 🔌 | **MCP Client** — async, stdio | Spawns and talks to the tool server |
| 🧰 | **MCP Tool Server** — FastMCP · `list_tools()` | Discovers and exposes every tool |
| 📚 | **RAG Agent** — Qdrant + BM25 + Cross-Encoder | Hybrid retrieval + reranked evidence search |
| 👁️ | **Vision Agent** — YOLOv8 + ByteTrack + VLM | Object detection, tracking, scene narration |
| 🗄️ | **Data Agent** — PostgreSQL + Celery | Incident records + async video jobs |
| 🕸️ | **Knowledge Agent** — Neo4j, parameterized Cypher | Multi-hop entity relationships |
| 🎙️ | **Audio Agent** — Whisper (STT) + pyttsx3 (TTS) | Transcription and spoken responses |
| 📝 | **Report Synthesis** | Agent composes findings from every tool into one report |
| 📊 | **Observability Layer** — Prometheus · Grafana · OpenTelemetry | Metrics + tracing across the whole loop |
| ✅ | **Human Review → Action** | Nothing ships without a human sign-off |
...
**✅ Verified live:** uploaded video evidence processed end-to-end through the dashboard — 150 frames analyzed, 2 tracked objects returned with real confidence scores — flowing straight into live Severity Distribution and Classification charts on the same screen. See the [demo above](#-live-demo).
...
├── assets/
│   ├── demo.gif                   # Dashboard demo (used at the top of this README)
